...
const VideoComponent: React.FC<{ hoverVideo: string, videoRef: React.RefObject<HTMLVideoElement> }> = ({ hoverVideo, videoRef }) => {
  useEffect(() => {
    if (videoRef.current) {
  ...
  return (
    ...
    >
      <div className="absolute inset-0">
        ...
        {hoverVideo && (
          <Suspense
            fallback={<Image src={image.asset.url} alt={title} fill sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" className="object-cover" />}
          >
            <VideoComponent hoverVideo={hoverVideo.asset.url} videoRef={videoRef} />
          </Suspense>
        )}
      </div>
      <div
